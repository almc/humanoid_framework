...
ea=0.5; % alpha value of the streamlines
...
if isempty(verts)
  if isempty(w)       % 2D
    ...
    if isempty(x)
      ...
      verts = stream3(x,y,z,u,v,w,sx,sy,sz,[0.01, 1000]) %[0.001, 5000]
      %verts = interpstreamspeed(x,y,z,u,v,w,verts,0.01);
    end
  end
end
  ...
  if ~isempty(vv)
    if size(vv,2)==3
      %h = [h ; line(vv(:,1), vv(:,2), vv(:,3))];
        ...
        Vcol(Vcol==0)=1;
    for j=2:length(X)
        % modification in order to be able to draw with alpha transparency, patchline vs line
        patchline([X(j-1) X(j)], [Y(j-1) Y(j)], [Z(j-1) Z(j)],'linestyle','-','edgecolor', [cmap(Vcol(j),1) cmap(Vcol(j),2) cmap(Vcol(j),3)],'linewidth',lw,'edgealpha',ea);
